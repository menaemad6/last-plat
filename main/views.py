...
# line 390 must be uncommented , commented due to vercel database restrictions to read only!
...
# def drm_solution(request , id):
#     video_id = id
#     video = Part.objects.get(id=video_id)
...
def login(request):
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            # Get user input from POST request safely
            username = request.POST.get('username')
            password = request.POST.get('password')

            device_type = request.POST.get('device-type')
            browser_type = request.POST.get('browser-type')

            # Check credentials using authenticate
            ...
            if user is not None:
                ...
                # Log the login info (avoid storing plaintext password)
                ...
                try:
                    ...
                    user = authenticate(request, username=profile_user.username, password=password)
...
# #     endpoint = f"{settings.BUNNYCDN_STORAGE_ENDPOINT}/{settings.BUNNYCDN_STORAGE_ZONE_NAME}/{file_name}"
# #     headers = {
# #         "AccessKey": settings.BUNNYCDN_STORAGE_ACCESS_KEY,
# #     }
...
# #         pull_zone_url = f"https://{settings.BUNNYCDN_STORAGE_ZONE_NAME}.b-cdn.net/{file_name}"
# #         return pull_zone_url
# #     else:
# #         raise Exception(f"Failed to upload to BunnyCDN: {response.status_code} {response.text}")
...
# #     """
# #     View to upload a video to BunnyCDN.
# #     """
# #     if request.method == "POST" and request.FILES.get("video"):
# #         video_file = request.FILES["video"]
# #         local_path = f"/tmp/{video_file.name}"  
...
# #         try:
# #             public_url = upload_to_bunnycdn(local_path, file_name)
...
#     return render(request, 'upload-cdn.html')
...
#     """
#     Generates a signed URL for uploading a file to BunnyCDN.
#     """
#     access_key = settings.BUNNYCDN_STORAGE_API_KEY
#     storage_zone = settings.BUNNYCDN_STORAGE_ZONE_NAME
#     storage_endpoint = settings.BUNNYCDN_STORAGE_ENDPOINT

#     # Construct the upload URL
#     url_path = f"/{storage_zone}/{file_name}"
#     upload_url = f"{storage_endpoint}{url_path}"

#     # Generate expiration time for the signed URL (e.g., valid for 1 hour)
#     expiration_time = int(time.time()) + 3600
#     string_to_sign = f"{expiration_time}\n{url_path}"

#     # Generate signature using HMAC and SHA256
#     signature = hmac.new(
#         key=bytes(access_key, 'utf-8'),
#         msg=bytes(string_to_sign, 'utf-8'),
#         digestmod=hashlib.sha256
#     ).hexdigest()

#     # Full signed URL
#     signed_url = f"{upload_url}?expires={expiration_time}&signature={signature}"
#     return signed_url

# # View to get the signed upload URL
# @csrf_exempt
# def get_bunnycdn_upload_url(request):
#     if request.method == "GET":
#         file_name = "test_video.mp4"  # Replace with dynamic file names as needed
#         upload_url = generate_signed_url(file_name)

#         return JsonResponse({"success": True, "upload_url": upload_url})
