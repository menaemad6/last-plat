        ...
        if user_profile.instructor == True:
            ...
            if AssignmentOpen.objects.filter(user=request.user ,assignment_id=assignment.assignment_id).first():
                if AssignmentSubmit.objects.filter(user=request.user ,assignment_id=assignment.assignment_id).first():
                    ...
                    student_assignment_open_object_to_shuffle_questions = AssignmentOpen.objects.get(user=request.user , assignment_id=assignment.assignment_id)



                    # If Assignment Shuffle Mode Is On 
                    if assignment.shuffleQuestions:


                        # Shuffle Questions If Not Shuffled 
                        if student_assignment_open_object_to_shuffle_questions.alterShuffle == False:
                            student_assignment_open_object_to_shuffle_questions.alterShuffle = True
                            student_assignment_open_object_to_shuffle_questions.save()

                            
                            # Shuffle Questions
                            shuffled_questions_objects = []

                            real_questions_objects = Answer.objects.filter(assignment_id=assignment.assignment_id , user=request.user)
                            for x in real_questions_objects:
                                shuffled_questions_objects.append(x)

                            random.shuffle(shuffled_questions_objects)

                            counter = 1
                            for x in shuffled_questions_objects:
                                x.virtual_number = counter
                                x.save()
                                counter = counter + 1

                    else:
                        # Put Virtual Numbers same as the real number 
                        if student_assignment_open_object_to_shuffle_questions.alterShuffle == False:
                            student_assignment_open_object_to_shuffle_questions.alterShuffle = True
                            student_assignment_open_object_to_shuffle_questions.save()
                    ...
                    questions = Answer.objects.filter(assignment_id=assignment.assignment_id , user=request.user).order_by('virtual_number')



                    current_question_virtual_number = 1
                    if Question.objects.filter(question_id=question_id).first():
                        ...
                        current_question_virtual_number = question.virtual_number
                        # for m in shuffled_questions_objects:
                        ...
                        if Question.objects.filter(assignment_id=assignment.assignment_id).first():
                            first_question = Answer.objects.get(assignment_id=assignment.assignment_id , user=request.user, virtual_number=1)
                    ...
                    # Button Mode 
                    if current_question_virtual_number == 1:
                        previous = False
                        next = True
                        btn_mode = 'first'
                    else:
                        if current_question_virtual_number==assignment.questions_count:
                            previous = True
                            next = False
                            btn_mode = 'last'
                        else:
...
def create_assignment(request):
    ...
    if request.method == 'POST':
        ...
        new_assignment.qr_code= 'qr-codes/' + str(new_assignment.assignment_id) + '.png'
        new_assignment.save()
        # Qr Generation
...
def assignment_functions(request):
    ...
    if request.method == 'POST':
        ...
        if str(function_type) == 'assignment-settings' :
            ...
            shuffleQuestions = request.POST.get('shuffleQuestions')
            ...
            if shuffleQuestions == 'on':
                shuffleQuestions_value = True
            else:
                shuffleQuestions_value = False
            ...
            assignment.shuffleQuestions=shuffleQuestions_value
            assignment.save()
        ...
        if str(function_type) == 'save-answer':
            ...
            if str(destination) == 'next':
                current_question_no = student_answer.virtual_number
                destination_question = Answer.objects.get(assignment_id=assignment.assignment_id , user=request.user, virtual_number=int(current_question_no + 1)) 
