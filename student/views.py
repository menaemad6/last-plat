...
from main.models import Profile , Lecture , StudentLectureObject, LectureCode ,  Part , StudentPartObject, Chapter , ChapterLecture , ChapterMember , Group , GroupMember , GroupLecture, GroupMessage ,GroupRequest, BuyLesson , BuyChapter
...
def chapters(request , pk):
    ...
    if ChapterMember.objects.filter(user=profile.user).first():
        member_chapters_objects = ChapterMember.objects.filter(user=profile.user)
        chapters = []
        for x in member_chapters_objects:
            if Chapter.objects.filter(id=x.chapter_id).first():
                chapter = Chapter.objects.get(id=x.chapter_id)
    ...
    return render(request, 'profile/chapters.html' , {'chapters':chapters ,'user_profile': user_profile ,'profile': profile, 'profile_object': profile_object, 'notifications' : notifications_count })


@login_required(login_url='login')
def chapter(request , pk , ch):
    if User.objects.filter(username=request.user.username).first():
        user_object = User.objects.get(username=request.user.username)
        user_profile = Profile.objects.get(user=user_object)
    ...
    if Chapter.objects.filter(id=ch).first():
        chapter = Chapter.objects.get(id=ch)

        chapter_lectures = []
        chapter_lectures_objects = ChapterLecture.objects.filter(chapter_id=ch).order_by('-added_at')
        ...
        if ChapterMember.objects.filter(user=profile_object , chapter_id=chapter.id).first():
            chapter_member_object = ChapterMember.objects.get(user=profile_object , chapter_id=chapter.id)
            status = 'purchased'
        else:
            status='non-purchased'
    ...
    return render(request, 'profile/chapter.html' , {'chapter':chapter , 'lectures':chapter_lectures , 'status':status,  'user_profile': user_profile ,'profile': profile, 'profile_object': profile_object, 'notifications' : notifications_count })
    ...
    if request.method == 'POST':
        ...
        if str(function_type) == 'send-question':
            ...
            if len(request.FILES) != 0:
                question_image = request.FILES['questionImageInput']
            else:
                question_image = ''
            ...
            if str(destination) == 'lecture':
                ...
                create_question = StudentQuestion.objects.create(lecture_id=lecture_id , lecture_title=lecture_title , user=student_user , user_name=student_profile.name , user_image=student_profile.image ,question=question ,question_image=question_image)
                messages.success(request, 'تم ارسال سؤالك بنجاح')
                return redirect("/lecture/" + str(lecture_id))


            if str(destination) == 'inbox':
                create_question = StudentQuestion.objects.create(user=student_user , user_name=student_profile.name , user_image=student_profile.image ,question=question ,question_image=question_image)
                messages.success(request, 'تم ارسال سؤالك بنجاح')
        ...
        if str(function_type) == 'send-reply':
            ...
            question_image_query = request.POST['question_image_query']
            if question_image_query == 'True':
                answer_image = request.FILES['questionImageInput']
            else:
                answer_image = ''


            audio_file_query = request.POST['audio_file_query']
            if audio_file_query == 'True':
                answer_record = request.FILES['audioFile']
            else:
                answer_record = ''
            ...
            if user_profile.instructor == True:
                if StudentQuestion.objects.filter(question_id=question_id).first():
                    ...
                    create_reply = StudentQuestionAnswer.objects.create(question_id=question.question_id , user=request.user , user_name=user_profile.name , answered_to_profile=student_profile , question_text = question.question , answer=reply , answer_image=answer_image , answer_record=answer_record)
                    create_reply.save()
